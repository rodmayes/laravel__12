...
const create = () => {
    ...
    form.post(route("lottery.results.store", props.result?.id), {
        ...payload,
        preserveScroll: true,
        onSuccess: () => {
            emit("close");
...
<template>
    <Dialog v-model:visible="props.show" position="top" modal :header="'Create ' + props.title" :style="{ width: '60rem' }" :closable="false">
