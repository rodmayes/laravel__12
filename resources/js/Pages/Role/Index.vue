    ...
    <app-layout>
        <div class="card">
            ...
            <Edit
                ...
                :permissions="props.permissions"
            />
            <Button v-show="can(['role.create'])" label="Create" @click="data.createOpen = true" icon="pi pi-plus" />
            <DataTable lazy :value="roles.data" paginator  :rows="roles.per_page" :totalRecords="roles.total" :first="(roles.current_page - 1) * roles.per_page"   @page="onPageChange"  tableStyle="min-width: 50rem">
                ...
                <Column :exportable="false" style="min-width: 12rem">
                    <template #body="slotProps">
                        <Button v-show="can(['role.update'])" icon="pi pi-pencil" outlined rounded class="mr-2"  @click="
                                                    (data.editOpen = true),
                                                        (data.role = slotProps.data)" />
                        <Button v-show="can(['role.delete'])" icon="pi pi-trash" outlined rounded severity="danger" @click="deleteDialog = true; data.role = slotProps.data" />
                    </template>
                </Column>
            </DataTable>
